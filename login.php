...
$sql="SELECT roles.role_name, user.username, user.password FROM user INNER JOIN roles ON (roles.role_id = user.role_id) WHERE user.username='$username' AND user.PASSWORD='$password'";
 ...
 $sql="SELECT roles.role_name, donor_account.username, donor_account.password FROM donor_account INNER JOIN roles ON (roles.role_id = donor_account.role) WHERE donor_account.username='$username' AND donor_account.PASSWORD='$password'";
$row=mysql_query($sql);
$result=mysql_fetch_assoc($row);
 ...
 if($count==1){
  ...
  break;
   case 'donor':
  header('location:profile_dn.php');
  break;
 }
 }else{
...
$_SESSION['msg']=array(
   'err'=>'Your PassWord or UserName is not Found'
   );
 }
}
?>
  ...
  <form class="form-horizontal" name="contact_form" id="contact_form" method="post" action="">
            ...
            <fieldset>
                <div class="form-group">
                    ...
                    <div class="col-lg-6">
                        <input type="text" value="" placeholder="User Name" id="username" class="form-control" name="username" required="" onclick="clr()">
                    </div>
                </div>
